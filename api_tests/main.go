...
func main() {
	...
	if err != nil {
		raw, err = os.ReadFile("../data.txt")
		if err != nil {
			fmt.Fprintf(os.Stderr, "failed to read data.txt: %v\n", err)
			os.Exit(1)
		}
	}
	...
	if err := os.MkdirAll("fixtures", 0755); err != nil {
		fmt.Fprintf(os.Stderr, "failed to create fixtures directory: %v\n", err)
		os.Exit(1)
	}

	for i := range 10 {
		req, err := gen.GenerateSummarize(LightConfig())
		if err != nil {
			fmt.Fprintf(os.Stderr, "failed to generate summarize light %d: %v\n", i, err)
			continue
		}
		if err := SaveToFile(req, fmt.Sprintf("fixtures/summarize_light_%d.json", i)); err != nil {
			fmt.Fprintf(os.Stderr, "failed to save summarize light %d: %v\n", i, err)
		}
	}
	for i := range 5 {
		req, err := gen.GenerateSummarize(DefaultConfig())
		if err != nil {
			fmt.Fprintf(os.Stderr, "failed to generate summarize default %d: %v\n", i, err)
			continue
		}
		if err := SaveToFile(req, fmt.Sprintf("fixtures/summarize_default_%d.json", i)); err != nil {
			fmt.Fprintf(os.Stderr, "failed to save summarize default %d: %v\n", i, err)
		}
	}
	for i := range 3 {
		req, err := gen.GenerateSummarize(HeavyConfig())
		if err != nil {
			fmt.Fprintf(os.Stderr, "failed to generate summarize heavy %d: %v\n", i, err)
			continue
		}
		if err := SaveToFile(req, fmt.Sprintf("fixtures/summarize_heavy_%d.json", i)); err != nil {
			fmt.Fprintf(os.Stderr, "failed to save summarize heavy %d: %v\n", i, err)
		}
	}
}
